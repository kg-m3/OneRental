...
import { Calendar, MapPin, HandCoins, Shield, ArrowLeft, X, Clock, AlertTriangle, CheckCircle } from 'lucide-react';
...
const EquipmentDetails = () => {
    ...
    const fetchEquipmentDetails = async () => {
      ...
      try {
        ...
        if (imagesError) {
          ...
          setEquipment({ ...equipment, images: images || [] });
        }
      } catch (err) {
  ...
  useEffect(() => {
    if (equipment?.rate && duration > 0) {
      const subtotal = equipment.rate * duration;
      const serviceFee = 0; // Placeholder - could be percentage later
  ...
  const handleBookingSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!equipment?.id || !user?.id) {
      setError('Equipment or user not found');
      return;
    }
    ...
    if (user.id === equipment.user_profiles?.id) {
    ...
    try {
      ...
      // Calculate totals
      const subtotal = days * equipment.rate;
      const serviceFee = 0; // Placeholder
        ...
        .insert([
          {
            ...
            user_id: user.id, // Using user_id as per current schema
  ...
  const isOwner = user?.id === equipment?.user_profiles?.id;
        ...
        {showSuccessAlert && (
          <div className="mb-6 bg-green-50 border border-green-200 rounded-lg p-4">
            <div className="flex items-start">
              ...
              <div>
                <h4 className="text-green-800 font-medium">Booking Request Sent!</h4>
                <p className="text-green-600 text-sm mt-1">
                  The equipment owner will review and respond to your request. You can track the status in your dashboard.
                </p>
                <button
                  onClick={() => navigate('/dashboard')}
                  className="mt-2 text-green-700 hover:text-green-800 font-medium text-sm underline"
                >
                  View in Dashboard
                </button>
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-lg shadow-lg">
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-lg mb-6">
                ...
                {isOwner ? (
                  <div className="px-6 py-3 bg-gray-300 text-gray-500 rounded-lg font-semibold cursor-not-allowed" title="You can't book your own equipment">
                    Your Equipment
                  </div>
                ) : (
                  <button
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold">{equipment?.user_profiles?.company_name || equipment?.user_profiles?.full_name}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <form onSubmit={handleBookingSubmit} className="p-6 space-y-4 min-h-[300px] max-h-[calc(90vh-120px)] overflow-y-auto">
              ...
              <div className="flex justify-end pt-4 px-6">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-3 bg-blue-900 text-white rounded-lg font-semibold hover:bg-blue-800 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed"
                >
